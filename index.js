...
app.get('/api/main/:tower', (req, res) => {
  let sql = "select s_no, tower, DATE_FORMAT(booking_date, '%d-%m-%Y') as booking_date, cb.unit_no, area_sqft, applicant_name, applicant_mob_no, applicant_email, coapplicant_name, coapplicant_mob_no, coapplicant_email, broker, plan, loan, round(nbp/area_sqft) as rate, nbp, gst, tbc, tdtd, ifnull(rwgst, 0) as rwgst, ifnull(round(if(rwgst*0.05>ifnull(rgst, 0)>0, rgst, rwgst*0.05)), 0) as rgst, ifnull(round(if(rwgst*0.05>ifnull(rgst, 0)>0, rwgst-rgst, rwgst-rwgst*0.05)), 0) as rwogst, round(ifnull(rwgst*100/tbc,0)) as rec_per, round(tbc-ifnull(rwgst,0)) as balance, round(tdtd-rwgst) as o_t, gst_choice, address, aadhar_card, pan_card from(select s_no, booking_date, tower, unit_no->>'$.unit_no' as unit_no, area_sqft->>'$.area_sqft' as area_sqft, applicant_name, applicant_mob_no, applicant_email, coapplicant_name, coapplicant_mob_no, coapplicant_email, broker->>'$.bcn' as broker, plan->>'$.plan' as plan, loan, round(if(nbp=0, tbc-tbc*5/105, nbp)) as nbp, round(if(nbp=0, tbc*5/105, nbp*0.05)) as gst, round(if(tbc=0, nbp+nbp*0.05, tbc)) as tbc, round(if(tbc=0, (nbp+nbp*0.05)*0.4, tbc*0.4)) as tdtd, gst_choice->>'$.gst_choice' as gst_choice, address, aadhar_card, pan_card from customer)cb left join (select unit_no, sum(rwgst) as rwgst, sum(rgst) as rgst from customer_account group by unit_no)cba on cb.unit_no=cba.unit_no where tower=" + req.params.tower + "order by DATE_FORMAT(booking_date, '%Y-%m-%d')";
  let query = conn.query(sql, (err, results) => {
    if (err) {
      throw err
    }
...
app.get('/api/main/:tower/:unit_no', (req, res) => {
  let sql = "select s_no, tower, DATE_FORMAT(booking_date, '%d-%m-%Y') as booking_date, cb.unit_no, area_sqft, applicant_name, applicant_mob_no, applicant_email, coapplicant_name, coapplicant_mob_no, coapplicant_email, broker, plan, loan, round(nbp/area_sqft) as rate, nbp, gst, tbc, tdtd, ifnull(rwgst, 0) as rwgst, ifnull(round(if(rwgst*0.05>ifnull(rgst, 0)>0, rgst, rwgst*0.05)), 0) as rgst, ifnull(round(if(rwgst*0.05>ifnull(rgst, 0)>0, rwgst-rgst, rwgst-rwgst*0.05)), 0) as rwogst, round(ifnull(rwgst*100/tbc,0)) as rec_per, round(tbc-ifnull(rwgst,0)) as balance, round(tdtd-rwgst) as o_t, gst_choice, address, aadhar_card, pan_card from(select s_no, booking_date, tower, unit_no->>'$.unit_no' as unit_no, area_sqft->>'$.area_sqft' as area_sqft, applicant_name, applicant_mob_no, applicant_email, coapplicant_name, coapplicant_mob_no, coapplicant_email, broker->>'$.bcn' as broker, plan->>'$.plan' as plan, loan, round(if(nbp=0, tbc-tbc*5/105, nbp)) as nbp, round(if(nbp=0, tbc*5/105, nbp*0.05)) as gst, round(if(tbc=0, nbp+nbp*0.05, tbc)) as tbc, round(if(tbc=0, (nbp+nbp*0.05)*0.4, tbc*0.4)) as tdtd, gst_choice->>'$.gst_choice' as gst_choice, address, aadhar_card, pan_card from customer)cb left join (select unit_no, sum(rwgst) as rwgst, sum(rgst) as rgst from customer_account group by unit_no)cba on cb.unit_no=cba.unit_no where tower=" + req.params.tower + " and cb.unit_no=" + req.params.unit_no + "order by DATE_FORMAT(booking_date, '%Y-%m-%d')";
  let query = conn.query(sql, (err, results) => {
    if (err) {
      throw err
    }
...
app.get('/api/demandR/:tower/:unit_no', (req, res) => {
  let sql = "select id, substring(unit_no,1,1) as tower, unit_no, min(DATE_FORMAT(due_date, '%d-%m-%Y')) as due_date, description, percentage, net_bsp, gst/2 as cgst, gst/2 as sgst, gst, net_due, recieved, if(sum(net_due-recieved)=null, 0, sum(net_due-recieved)) as pending_amount from customer_payment_plan where due_date is not null and recieved<net_due and unit_no=" + req.params.unit_no;
  let query = conn.query(sql, (err, results) => {
    if (err) {
      throw err
    }
...
app.get('/api/total/:tower/:unit_no', (req, res) => {
  let sql = "select 'TOTAL' as description, due_date as due_date, sum(net_bsp) as net_bsp, sum(gst/2) as cgst, sum(gst/2) as sgst, sum(gst) as gst, sum(net_due) as net_due, sum(recieved) as recieved, if(sum(pending_amount)=null, 0, sum(pending_amount)) as pending_amount from (select id, substring(unit_no,1,1) as tower, unit_no, DATE_FORMAT(due_date, '%d-%m-%Y') as due_date, description, percentage, net_bsp, gst/2 as cgst, gst/2 as sgst, gst, net_due, recieved, net_due-recieved as pending_amount from customer_payment_plan where recieved=net_due and substring(unit_no,1,1)=" + req.params.tower + " and unit_no=" + req.params.unit_no + " union select id, substring(unit_no,1,1) as tower, unit_no, DATE_FORMAT(due_date, '%d-%m-%Y') as due_date, description, percentage, net_bsp, gst/2 as cgst, gst/2 as sgst, gst, net_due, recieved, net_due-recieved as pending_amount from customer_payment_plan where due_date<current_date() and due_date is not null and recieved<net_due and substring(unit_no,1,1)=" + req.params.tower + " and unit_no=" + req.params.unit_no + " union select id, substring(unit_no,1,1) as tower, unit_no, DATE_FORMAT(due_date, '%d-%m-%Y') as due_date, description, percentage, net_bsp, gst/2 as cgst, gst/2 as sgst, gst, net_due, recieved, 0-recieved as pending_amount from customer_payment_plan where due_date>=current_date() and due_date is not null and recieved<net_due and substring(unit_no,1,1)=" + req.params.tower + " and unit_no=" + req.params.unit_no + " ORDER BY id DESC) main";
  let query = conn.query(sql, (err, results) => {
    if (err) {
      throw err
    }
